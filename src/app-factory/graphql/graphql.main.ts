...
//   const app: INestApplication<any> = await NestFactory.create(MyGraphQLModule, {
...
export async function bootstrap() {
  try {
    ...
    httpApp.listen(httpOptions.port, () => {
      ...
      console.log(
        `GraphQL Playground is running on: http://${httpOptions.host}:${httpOptions.port}/graphql`,
      );
    });
    ...
    if (httpsEnabled === true && httpsApp !== undefined) {
      ...
      httpsApp?.listen(httpsOptions.port, () => {
        ...
        console.log(
          `GraphQL Playground is running on: https://${httpsOptions.host}:${httpsOptions.port}/graphql`,
        );
      });
    }
  } catch (error) {
